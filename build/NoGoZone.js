...
function CreateWindow() {
    ...
    var groupbox = new Oui.GroupBox("Set up a no go zone");
    window.addChild(groupbox);

    var startButton = new Oui.Widgets.Button("Start", function () {
        defineNoGoZone();
    });

    var clearButton = new Oui.Widgets.Button("Clear all", function () {
        clearNoGoZone();
    });
...
function clearNoGoZone(entity) {
    for (var i = 0; i < map.numEntities; i++) {
        var _entity = map.getEntity(i);

        if (_entity && _entity.peepType && _entity.peepType !== 'guest') {
            console.log(_entity.patrolArea);
            _entity.patrolArea.clear();
        }
    }
}
...
function getAllTiles() {
    ...
    for (var x = 5; x < map.size.x - 5; x++) {
        for (var y = 5; y < map.size.y - 5; y++) {
            var tile = map.getTile(x, y);
            var nbElements = tile.numElements;
